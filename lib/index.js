...
export async function apply(ctx, config, options = {}) {
  ...
  const status = (contextId) => {
    ...
    // explicitNone (issue #13): a user-explicit "active remote = none" must
    // also suppress the config-default DISPLAY — activeMachineOf already
    // returns null for the effective machine here, so the display has to agree
    // (host/port/username from no machine, activeSource 'none') instead of
    // claiming the config default is active. Fresh globals (no explicit
    // choice) keep the bootstrap display; bound/ephemeral contexts are
    // unaffected (machine branch).
    const cfgGlobal = isGlobal && !store.explicitNone
    const host = machine ? String(machine.host || '') : (cfgGlobal ? String(config.host || '') : '')
    const port = machine ? (Number(machine.port) || 22) : (cfgGlobal ? (Number(config.port) || 22) : 22)
    const username = machine ? String(machine.username || '') : (cfgGlobal ? String(config.username || '') : '')
    const workspace = entry.workspace || (machine && machine.workspace) || ''
    const hkHost = machine ? String(machine.host || '') : (cfgGlobal ? String(config.host || '') : '')
    const hkPort = machine ? (Number(machine.port) || 22) : (cfgGlobal ? (Number(config.port) || 22) : 22)
    ...
    const activeSource = entry.ephemeralMachine ? 'ephemeral'
      ...
      : (isGlobal ? (store.currentId ? 'context' : (cfgGlobal && config.host ? 'config' : 'none')) : 'none'))
